...
function checkForPawnPromtion(piece, id) {
  if (inTurn === "white") {
    if (
...
function pawnPromotionCallback(piece, id) {
  ...
  const currentELemenet = document.getElementById(id);
  currentELemenet.innerHTML = "";
  currentELemenet.append(image);
}

// move element to square with id
function moveElement(piece, id) {
  const pawnISPromoted = checkForPawnPromtion(piece, id);
  ...
  piece.current_position = id;
  if (pawnISPromoted) {
    pawnPromotion(inTurn, pawnPromotionCallback, id);
  }
  checkForCheck();
  changeTurn();
  // globalStateRender();
